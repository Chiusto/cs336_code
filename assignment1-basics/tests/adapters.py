...
def run_multihead_self_attention(
    ...
    mha = multihead_self_attention(d_model, num_heads, in_features.shape[-2], theta=None)
...
def run_transformer_block(
    ...
    """
    from cs336_basics.model.transformer_block import Transformer_block
    ...
    block.attention.q_proj.weight.data = weights["attn.q_proj.weight"]
    block.attention.k_proj.weight.data = weights["attn.k_proj.weight"]
    block.attention.v_proj.weight.data = weights["attn.v_proj.weight"]
    block.attention.o_proj.weight.data = weights["attn.output_proj.weight"]
...
def run_transformer_lm(
    ...
    model.rmsnorm.weight.data = weights["ln_final.weight"]
    model.output_embedding.weight.data = weights["lm_head.weight"]
    for i in range(num_layers):
        prefix = f"layers.{i}."
        block = model.transformer_layers[i]
        block.rmsnorm1.weight.data = weights[prefix + "ln1.weight"]
        block.rmsnorm2.weight.data = weights[prefix + "ln2.weight"]
        block.attention.q_proj.weight.data = weights[prefix + "attn.q_proj.weight"]
        block.attention.k_proj.weight.data = weights[prefix + "attn.k_proj.weight"]
        block.attention.v_proj.weight.data = weights[prefix + "attn.v_proj.weight"]
        block.attention.o_proj.weight.data = weights[prefix + "attn.output_proj.weight"]
        block.SwiGLU.w1.weight.data = weights[prefix + "ffn.w1.weight"]
        block.SwiGLU.w2.weight.data = weights[prefix + "ffn.w2.weight"]
        block.SwiGLU.w3.weight.data = weights[prefix + "ffn.w3.weight"]
...
def run_get_batch(
    ...
    """
    from cs336_basics.train.data_loading import data_loading
